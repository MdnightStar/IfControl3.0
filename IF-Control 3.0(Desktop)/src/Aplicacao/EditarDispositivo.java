 ...
 * @author cauaa
 */
public class EditarDispositivo extends javax.swing.JFrame {
    ...
    public EditarDispositivo( Dispositivo dis) {
        initComponents();
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        ...
        gson=new Gson();
        id=dis.getId();
        jTextFieldMarca.setText(dis.getMarca());
        jTextFieldModelo1.setText(dis.getModelo());
        String[] config=extrairValores(dis.getConfig());
        jTextFieldOneSpace.setText(config[0]);
        jTextFieldZeroSpace.setText(config[1]);
        jTextFieldBitMark.setText(config[2]);
        jTextFieldPauseSpace.setText(config[3]);
        jTextFieldHeaderMark.setText(config[4]);
        jTextFieldHeaderSpace.setText(config[5]);
        if(dis.getTipo().contains("AR")){
            jCheckBoxAR.setSelected(true);
            codAr = new AddCodigosAr(dis.getListaCodigos());
        }else{
            jCheckBoxDS.setSelected(true);
            codDs = new AddCodigosDs(dis.getListaCodigos());
        }
        jTextFieldSalasRe.setText(Arrays.stream(dis.getSalasRelacionadas()).boxed().map(Object::toString).collect(Collectors.joining(", ")));
    ...
    public static String[] extrairValores(String texto) {
        ...
        // 1. Remove os colchetes de abertura e fechamento
        String textoLimpo = texto.replace("[", "").replace("]", "");

        // 2. Divide a string baseada na vírgula
        String[] resultados = textoLimpo.split(",");
    ...
    public void terminar(Dispositivo dis){
        ...
        String resp=MainApp.sessao.trataAcao("--editDispositivo--"+envio);
    ...
    private void initComponents() {
        ...
        jLabelAdicionarDispositivo1.setText("Editar Dispositivo");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(EditarDispositivo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new EditarDispositivo().setVisible(true);
            }
        });
    }
